...
fn run_high_eviction() {
    ...
    println!(
        "   - Map Capacity Maintained: {} (Evictions: {})",
        map.capacity(),
        map.eviction_count()
    );
}
...
fn run_vip_protection() {
    ...
    for vip_id in 0..10 {
        if map.get(&vip_id).is_some() {
            vips_survived += 1;
        }
    }
    println!(
        "   - VIP Survival Rate: {}/10 against cache pollution",
        vips_survived
    );
}
...
fn run_stress_test() {
    ...
    println!(
        "   - Stress test complete. Hits: {}, Evictions: {}, Time: {:.2?}",
        total_hits,
        map.eviction_count(),
        start.elapsed()
    );
